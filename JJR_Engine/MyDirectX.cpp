...
char keys[256];
int prevKey = NULL; // keeps the player from spamming
int prevBut = NULL; // same as above but for the Xbox360 controller
XINPUT_GAMEPAD controllers[4];
//controller vibration
int vibrating = 0;
int vibration = 100;
int controllerInputs[] = { XINPUT_GAMEPAD_DPAD_UP,
...
{
	if (prevKey != keys[key] && (keys[key]) & 0x80 )
	{
		prevKey = keys[key];
		return true;
	}
...
{
	return controllers[0].wButtons & button;
}

bool Button_Down_Once(int button)
{
	if (prevBut != button && (controllers[0].wButtons & button))
	{
		prevBut = button;
		return true;
	}
...
bool noButton()
{
	...
	{
		if (controllers[0].wButtons & controllerInputs[i])
			return false;
	}
	prevBut = NULL;
